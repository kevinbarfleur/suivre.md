...
up with no other key or modifier in between; two clean taps within 300 ms fire. The
...
starts working the moment you allow it, no relaunch.
...
Servers are launched one per project through a **login shell**
(`/bin/zsh -lc … exec tsx src/cli/index.ts board --port <p>`) so that nvm/homebrew
`node` is on PATH even when the app was double-clicked and inherits no shell
environment. `exec` means the shell is replaced by the server process, so stopping
it is a clean signal to the real thing.
...
- **The web view is torn down after 10 minutes hidden.** Idle, before you've
  ...
  minutes and it's released again, back to that baseline. Sessions survive because
  they're on disk, so the next summon just reloads.
...
Measured: ~28 MB idle (no WebContent process), ~88 MB RSS with a page loaded (the
shared WebKit process is separate and reclaimed on unload), 0 leaks.
...
swift run suivre-desktop --selftest # headless: spawn a server, check health, quit
